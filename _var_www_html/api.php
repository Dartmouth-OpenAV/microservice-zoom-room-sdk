...
function instantiate_process_for_device_if_needed($device)
{
    ...
    if (file_exists("/dev/shm/{$device}.pid")) {
        ...
        if (
            !posix_kill(
                intval(trim(file_get_contents("/dev/shm/{$device}.pid"))),
                0
            )
        ) {
            $instantiate = true;
    ...
    if ($instantiate) {
        ...
        shell_exec(
            "/fifo_runner.sh /controller/app/OpenAVControllerApp {$device} /dev/shm/{$device} > /dev/shm/{$device}.stdout 2>/dev/shm/{$device}.stderr &"
        );
    }

    $sleep_counter = 0;
    while ($sleep_counter < 30 && !file_exists("/dev/shm/{$device}.pid")) {
        usleep(100000);
        $sleep_counter++;
    }
    if (!file_exists("/dev/shm/{$device}.pid")) {
    ...
    if (
        ...
        add_error(
            $device,
            "unable to instantiate controller app (pid file exists but process isn't up)"
        );
        return false;
    }
...
if (is_cli()) {
    if (isset($argv[1])) {
        $id = $argv[1];
        $task = sqlite_query("SELECT * FROM tasks WHERE id=:id", [
            ":id" => $id,
        ]);
        if (count($task) == 1) {
            $task = $task[0];
            ...
            if ($task["method"] == "GET") {
                $function_name .= "get_";
            } elseif ($task["method"] == "PUT") {
                $function_name .= "set_";
            } elseif ($task["method"] == "DELETE") {
                $function_name .= "unset_";
            }
            $function_name .= str_replace("/", "_", $task["path"]);

            if (function_exists($function_name)) {
                $instantiation_result = instantiate_process_for_device_if_needed(
                    $task["device"]
                );
            ...
            sqlite_query(
                "UPDATE data SET last_refreshed_timestamp=CURRENT_TIMESTAMP WHERE device=:device AND
                                                                                                        path=:path",
                [":device" => $task["device"], ":path" => $task["path"]]
            );
        } else {
        ...
        while (true) {
            $devices_currently_in_process = sqlite_query(
                "SELECT DISTINCT(device) AS distinct_device FROM tasks WHERE in_process='true'",
                []
            );
            $devices_currently_in_process_for_query = [];
            foreach (
                $devices_currently_in_process
                as $device_currently_in_process
            ) {
                $devices_currently_in_process_for_query[] =
                    $device_currently_in_process["distinct_device"];
            }
            $devices_currently_in_process_for_query =
                "'" .
                implode("','", $devices_currently_in_process_for_query) .
                "'";
            ...
            $query .=
                "in_process='false' AND (method='PUT' OR method='PATCH' OR method='DELETE') AND process_at_timestamp<=CURRENT_TIMESTAMP ORDER BY added_timestamp, id ASC LIMIT 1";
            ...
            if (count($task) == 0) {
                ...
                $query .=
                    "in_process='false' AND method='GET' AND process_at_timestamp<=CURRENT_TIMESTAMP ORDER BY added_timestamp, id ASC LIMIT 1";
                $task = sqlite_query($query, []);
            }
            ...
            if (count($task) > 0) {
                ...
                $id = $task[0]["id"];
                sqlite_query(
                    "UPDATE tasks SET in_process='true',
                                                in_process_since_timestamp=CURRENT_TIMESTAMP WHERE id=:id",
                    [":id" => $id]
                );
                echo "> " .
                    date("Y-m-d H:i:s") .
                    " - forking task {$id}, log @ /var/log/task.{$id}.log\n";
                shell_exec(
                    "/usr/bin/timeout 9 php /var/www/html/api.php {$id} > /var/log/task.{$id}.log 2>&1 &"
                );
            } else {
                ...
                if ($maintenance_every_counter == 0) {
                    ...
                    sqlite_query(
                        "DELETE FROM data WHERE last_queried_timestamp<DATETIME('now', '-{$refresh_for_how_long} minutes')",
                        []
                    );
                    $obsolete_tasks = sqlite_query(
                        "SELECT * FROM tasks WHERE in_process='true' AND
                    ...
                    if (count($obsolete_tasks) > 0) {
                        add_error(
                            "all",
                            "found obsolete tasks: " .
                                json_encode($obsolete_tasks, JSON_PRETTY_PRINT)
                        );
                        sqlite_query(
                            "DELETE FROM tasks WHERE in_process='true' AND
                                                               (`in_process_since_timestamp`<DATETIME('now', '-20 seconds') OR in_process_since_timestamp IS NULL)",
                            []
                        );
                    }
                    ...
                    $data_still_being_queried = sqlite_query(
                        "SELECT * FROM data WHERE no_refresh='false' AND
                    ...
                    foreach ($data_still_being_queried as $datum) {
                        $in_tasks_count = sqlite_query(
                            "SELECT COUNT(1) FROM tasks WHERE device=:device AND
                                                                                          path=:path AND
                                                                                          method='GET'",
                            [
                                ":device" => $datum["device"],
                        ...
                        if ($in_tasks_count === 0) {
                            echo "> " .
                                date("Y-m-d H:i:s") .
                                " - adding task GET {$datum["device"]}/{$datum["path"]}\n";
                            sqlite_query(
                                "INSERT INTO tasks (device,
                                                              path,
                    ...
                    if (mt_rand(0, 99) == 0) {
                        echo "> " .
                            date("Y-m-d H:i:s") .
                            " - removing task log files older than 10 days\n";
                        shell_exec(
                            '/usr/bin/find /var/log -name "task.*.log" -type f -mtime +10 -exec rm \{\} \\;'
                        );
                    }
                    ...
                    if (mt_rand(0, 99) == 0) {
                        echo "> " .
                            date("Y-m-d H:i:s") .
                            " - killing controller processes no longer tied to a device\n";
                        $devices_in_db = sqlite_query(
                            "SELECT DISTINCT(device) AS device FROM data",
                            []
                        );
                        ...
                        foreach ($pid_files as $pid_file) {
                            ...
                            if (!$found) {
                                $pid = intval(
                                    trim(file_get_contents($pid_file))
                                );
                                $ppid = get_parent_pid($pid);
                                echo "> " .
                                    date("Y-m-d H:i:s") .
                                    " - killing pid: {$pid} & ppid: {$ppid} for device: {$device}\n";
...
header(
    "Access-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT,PATCH,DELETE,EXPORT"
);
header(
...
if ($path == "meeting/muted" && $method == "GET") {
    get();
}
if ($path == "meeting/muted" && $method == "PUT") {
    set();
}

if ($path == "meeting/video_muted" && $method == "GET") {
    get();
}
if ($path == "meeting/video_muted" && $method == "PUT") {
    set();
}

if ($path == "meeting/status" && $method == "GET") {
    get_hierarchy();
}

if ($path == "meeting/info" && $method == "GET") {
    get_hierarchy();
}

if ($path == "meeting" && $method == "GET") {
    get_hierarchy();
}
if ($path == "meeting" && $method == "PUT") {
    set();
}
if ($path == "meeting" && $method == "DELETE") {
    delete();
}
...
function get()
{
    ...
    $entry_count = sqlite_query(
        "SELECT COUNT(1) FROM data WHERE device=:device AND
                                                                  path=:path",
        [$device, $path],
        true
    );
    ...
    if ($entry_count > 0) {
        if ($entry_count > 1) {
            ...
            sqlite_query(
                "DELETE FROM data WHERE device=:device AND
                                                  path=:path LIMIT " .
                    ($entry_count - 1),
                [$device, $path]
            );
        ...
        sqlite_query(
            "UPDATE data SET last_queried_timestamp=CURRENT_TIMESTAMP WHERE device=:device AND
                                                                                      path=:path",
            [$device, $path]
        );
...
function set($no_refresh = false)
{
    ...
    sqlite_query(
        "DELETE FROM tasks WHERE device=:device AND
                                           path=:path AND
                                           method=:method",
        [":device" => $device, ":path" => $path, ":method" => $method]
    );
    if ($no_refresh === "false") {
        sqlite_query(
            "INSERT INTO tasks (device,
...
function set_meeting($device, $data)
{
    if (
        ...
        add_error(
            $device,
            "erroneous data provided to join_meeting with: " .
                json_encode($data)
        );
    } else {
        ...
        while ($meeting_status === false) {
            $meeting_status_count = sqlite_query(
                "SELECT COUNT(1) FROM data WHERE device=:device AND
                                                                                   path=:path",
                [$device, "meeting/status"],
                true
            ...
            if ($meeting_status_count == 0) {
                ...
                if (
                    ...
                    file_put_contents(
                        "/dev/shm/{$device}.fifo",
                        "send_meeting_passcode {$data["meeting_passcode"]}\n"
                    );
                } elseif ($meeting_status == "in_meeting") {
                    // all right!
                    sqlite_query(
                        "INSERT INTO data (device,
                                               path,
...
function close_with_500($message)
{
    ...
    $to_return = ["success" => false, "message" => $message];
...
function get_device_info($device)
{
    ...
    if (count($device) == 1) {
        return ["username" => "", "password" => "", "fqdn" => $device[0]];
    } elseif (count($device) == 2) {
        ...
        if (count($credentials) == 2) {
            ...
            add_error(
                "all",
                "unparseable device credentials: {$device_original}"
            );
        }
    } else {
